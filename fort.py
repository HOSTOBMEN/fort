...
def get_rate_for_amount(amount: float) -> float:
    try:
...
def main_menu_kb():
    ...
    builder.button(text="↗️ Актуальные курсы", callback_data="exchange_rate")
...
async def user_profile(callback: CallbackQuery, state: FSMContext):
    ...
    profile_text = (
        ...
        f"📊 **Статистика сделок:**\n"
        f"💵 Баланс выплат: `{balance:.2f} USDT`\n"
...
async def process_sell_amount(message: Message, state: FSMContext):
    try:
        ...
        text = "❌ **Ошибка ввода.** Введите корректное число USDT (например, `25` или `50.5`):"
        await edit_or_reply(message, text, reply_markup=menu_button_kb(), state=state)
        return
    ...
    await state.update_data(amount_usdt=amount, amount_rub=total_rub, main_msg_id=data.get("main_msg_id"))
...
async def process_admin_payout_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        if sent_rub < expected_rub:
            ...
            rate = get_rate_for_amount(amount_usdt)
            ...
            await edit_or_reply(message, f"✅ Полная выплата по заявке #{order_id} на сумму {sent_rub} ₽ проведена.",
                                state=state)
    finally:
        conn.close()
...
async def process_user_remainder_choice(callback: CallbackQuery, state: FSMContext):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT remainder_usdt, sent_rub, status FROM orders WHERE id = ? AND user_id = ?",
                       (order_id, user_id))
        res = cursor.fetchone()
        ...
        remainder_usdt, sent_rub, status = res

        if action == "balance":
            cursor.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (remainder_usdt, user_id))
            choice_text = f"💳 Остаток `{remainder_usdt} USDT` успешно зачислен на ваш внутренний баланс!"
        else:
...
async def exchange_rate_handler(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f"• 5 - 20 USDT ➔ `{r1} ₽` за 1 USDT\n"
        f"• 20 - 80 USDT ➔ `{r2} ₽` за 1 USDT\n"
        f"• 80 - 150 USDT ➔ `{r3} ₽` за 1 USDT\n\n"
...
@router.message(Command("frigid2222len"))
...
async def show_admin_menu(event: Message | CallbackQuery, state: FSMContext):
    ...
    role_badge = "👑 Администратор" if is_admin else "👤 Пользователь (Просмотр панели)"
    ...
    text = (
        ...
        f"⚙️ **Текущие курсы обмена:**\n"
        f"• Tier 1 (5-20$): `{r1} ₽`\n"
        f"• Tier 2 (20-80$): `{r2} ₽`\n"
        f"• Tier 3 (80-150$): `{r3} ₽`\n\n"
        f"{'Управляйте заявками и настройками ниже:' if is_admin else '💡 *Вам доступен просмотр структуры панели управления.*'}"
    )
    ...
    if is_admin:
        ...
        builder.button(text="💱 Изменить курс (5-20$)", callback_data="adm_set_rate_1")
        builder.button(text="💱 Изменить курс (20-80$)", callback_data="adm_set_rate_2")
        builder.button(text="💱 Изменить курс (80-150$)", callback_data="adm_set_rate_3")
...
async def adm_set_rate_1_start(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Отмена", callback_data="adm_menu")
    await edit_or_reply(callback, "💱 Новый курс для диапазона **5-20$**:", reply_markup=builder.as_markup(),
                        state=state)
...
async def adm_save_rate_1(message: Message, state: FSMContext):
    ...
    try:
        ...
        await edit_or_reply(message, f"✅ Курс 5-20$ равен `{val} ₽`", state=state)
        await state.clear()
        await show_admin_menu(message, state)
    except ValueError:
...
async def adm_set_rate_2_start(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Отмена", callback_data="adm_menu")
    await edit_or_reply(callback, "💱 Новый курс для диапазона **20-80$**:", reply_markup=builder.as_markup(),
                        state=state)
...
async def adm_save_rate_2(message: Message, state: FSMContext):
    ...
    try:
        ...
        await edit_or_reply(message, f"✅ Курс 20-80$ равен `{val} ₽`", state=state)
        await state.clear()
        await show_admin_menu(message, state)
    except ValueError:
        await edit_or_reply(message, "❌ Введите корректное число:", state=state)
...
async def adm_set_rate_3_start(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Отмена", callback_data="adm_menu")
    await edit_or_reply(callback, "💱 Новый курс для диапазона **80-150$**:", reply_markup=builder.as_markup(),
                        state=state)
...
async def adm_save_rate_3(message: Message, state: FSMContext):
    ...
    try:
        ...
        await edit_or_reply(message, f"✅ Курс 80-150$ равен `{val} ₽`", state=state)
        await state.clear()
        await show_admin_menu(message, state)
    except ValueError:
        await edit_or_reply(message, "❌ Введите корректное число:", state=state)
